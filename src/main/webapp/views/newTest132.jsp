...
<script type="text/javascript">
    ...
    function count() {
        for (var i = 1; i <= questionSum; i++) {
            ...
            if([5,12,14,18,20,21,30,31,36,38,40,41,46,47,49,50,52,53,59,60,64,67,68,82,83,86,97,99,114,116].indexOf(i)!= -1){
                questionScore[1] += temp;
                continue;
            }
            if([8,10,17,19,29,32,45,55,76,80,90,106,110,113,123,127,130,132].indexOf(i)!= -1){
                questionScore[2] += temp;
                continue;
            }
            if([2,7,16,33,35,51,56,62,65,73,77,88,93,103,108,111,120,125,128].indexOf(i)!= -1){
                questionScore[3] += temp;
                continue;
            }
            if([1,3,11,15,25,28,37,39,57,58,61,66,72,74,84,91,94,95,101,107,118,124].indexOf(i)!= -1){
                questionScore[4] += temp;
                continue;
            }
            if([4,6,9,13,23,24,34,43,48,54,63,69,78,79,89,92,96,102,104,112,119,121,129,131].indexOf(i)!= -1){
                questionScore[5] += temp;
                continue;
            }
            if([22,26,27,42,44,70,71,75,81,85,87,98,100,105,109,115,117,122,126].indexOf(i)!= -1){
